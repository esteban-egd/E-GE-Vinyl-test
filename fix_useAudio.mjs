...
const cleanPlay = `  const play = useCallback((rawTrack) => {
    if (!rawTrack) return;
      ...
      (async () => {
        try {
          ...
          if (bestMatch && bestMatch.videoId && bestScore > -2000) {
            ...
            // <-- UPDATE THE STATE HERE SO WATCHDOG KNOWS THE ID -->
            setCurrentTrack({...trackMeta});
...
content = content.slice(0, playStart) + cleanPlay + content.slice(playEnd);
